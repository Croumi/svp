...
def main(originalFile, anonymisedFile, return_dict, parameters={"size":2,"nbPOI":1,"night_start":22,"night_end":6,"work_start":9,"work_end":16,"weekend_start":10,"weekend_end":18}):
	global size
	...
	for id in final_tab_original:
		for type in final_tab_original[id]:
			for gps in final_tab_original[id][type]:
				time_second_original = final_tab_original[id][type][gps].total_seconds()
				time_second_anonymised = final_tab_anonymised[id][type][gps].total_seconds()
	...
	return_dict['utility_POI_perWeek'] = score/total_size
