...
### Méthode dépréciée ###
#--- equation "minimized_week_anonymised.csv"
#b_eq1 = 1/(1 - tabOri[key]/(tabOri[key]+max_offset))
#a_eq1 = -b_eq1/(tabOri[key]+max_offset)
#if tabAno[key] > tabOri[key]:
#	score = a_eq1*tabAno[key] + b_eq1
	#print(str(a_eq2)+"*x +"+str(b_eq2) + " - value of x= "+str(tabAno[key])+ " score= " +str(score))
#	if score < 0:
#		utility += 0
#	else:
#		utility+= score
#else:
#	tmp = (tabOri[key]-tabAno[key])+tabOri[key]
#	score = a_eq1*tmp + b_eq1
	#print(str(a_eq1)+"*x +"+str(b_eq1) + " - value of x= "+str(tabAno[key])+ " score= " +str(score))
	# ---- Autre méthode ----
	#b_eq2 = 0
	#a_eq2 = 0
	#if tabOri[key]-max_offset == 0:
	#	a_eq2 = 1/tabOri[key]
	#else:
	#	b_eq2 = 1/(1 - tabOri[key]/(tabOri[key]-max_offset))
	#	a_eq2 = -b_eq2/(tabOri[key]-max_offset)
	#score = a_eq2*tabAno[key]+ b_eq2
	#print(str(a_eq2)+"*x +"+str(b_eq2) + " - value of x= "+str(tabAno[key])+ " score= " +str(score))
#	if tabAno[key]==tabOri[key]:
#		score= round(score)
#	if score < 0:
